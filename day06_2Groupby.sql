...
-- yukaridaki kod mac'lerde hata verdi
-- windows'larda calisti
-- bu sogulama yanlis olur, group by ile tabloda gecen isimleri aliriz.




-- SORU02: Ülkelere göre ortalama maaşları listeleyiniz
select ulke, round(avg(maas)) as ortalama_maas from personel group by ulke;
...
select cinsiyet, count(*) as calisan_sayisi, round(avg(yas)) as ortalama_yas from personel group by ulke;


-- SORU08: Her ulke için bay ve bayan çalışan sayısı ve yaş ortalamasını  ve maası 30000 den büyük olanları sorgulayınız.
select ulke, cinsiyet, count(*) as calisan_sayisi, round(avg(yas)) as ortalama_yas, maas from personel where  maas>30000 group by ulke, cinsiyet;
...
select ulke, cinsiyet,count(*) as kisi, round(avg(yas)) as yas_ortalamasi from personel where maas>30000 group by ulke, cinsiyet order by yas_ortalamasi desc;
...
select isim, urun_adi, sum(urun_miktari) as toplam_urun from manav group by isim, urun_adi having toplam_urun>=3  order by toplam_urun desc;
-- aggregate fonksiyon kullandiysak, group by dan sonra where yerine having kullanacagiz.
-- where kullanamayi çünkü sum(urun_miktari) as toplam_urun
...
select urun_adi, max(urun_miktari) from manav group by urun_adi having max(urun_miktari) <> min(urun_miktari) order by max(urun_miktari);
